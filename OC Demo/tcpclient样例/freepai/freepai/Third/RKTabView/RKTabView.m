...
- (UIFont *)titlesFont {
    if (!_titlesFont) {
        _titlesFont = [UIFont systemFontOfSize:15];
    }
    return _titlesFont;
}
...
- (void)setTabContent:(UIControl *)tab withTabItem:(RKTabItem *)tabItem {
    ...
    //Subviews frames
    if (titleLabel) {
        if (tabItem.imageDisabled && tabItem.imageEnabled) {
            ...
            titleLabel.center = CGPointMake(tab.bounds.size.width/2, tab.bounds.size.height/2);
    ...
    if (self.darkensBackgroundForEnabledTabs) {
        if (tabItem.tabState == TabStateEnabled) {
            ...
            self.imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, tab.frame.size.width-2, tab.frame.size.height)];
            self.imageView.image = [UIImage imageNamed:@"u42"];
            self.imageView.center = CGPointMake(tab.bounds.size.width/2, tab.bounds.size.height/2);
            [[tab viewWithTag:DARKER_BACKGROUND_VIEW_TAG] addSubview:self.imageView];
