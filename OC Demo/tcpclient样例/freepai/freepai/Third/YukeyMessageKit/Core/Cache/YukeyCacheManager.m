...
#import "NSString+YukeyMD5.h"

@interface YukeyCacheManager () {
...
@implementation YukeyCacheManager
+ (instancetype)shareCacheManager {
    return self.manager;
}

+ (YukeyCacheManager *)cacheManagerWithIdentifier:(NSString *)identifier {
    return [[YukeyCacheManager alloc] initWithIdentifier:identifier];
}
...
+ (YukeyCacheManager *)manager {
    static YukeyCacheManager *sharedInstance = nil;
    static dispatch_once_t  onceToken;
    
    dispatch_once(&onceToken, ^{
        sharedInstance = [[YukeyCacheManager alloc] initWithIdentifier:NSStringFromClass(self)];
    });
    ...
    for(int i = 0; i < 16; i ++) {
        for(int j = 0; j < 16; j++) {
            ...
            for(id name in list) {
                ...
                if(attr) {
                    [files addObject:[[YukeyFileAttribute alloc] initWithPath:filePath attributes:attr]];
                }
            }
        }
    }
...
- (void)limitNumberOfCacheFiles:(NSInteger)numberOfCacheFiles {
    ...
    for(NSInteger i = numberOfCacheFiles; i < list.count; ++i) {
        YukeyFileAttribute *file = list[i];
