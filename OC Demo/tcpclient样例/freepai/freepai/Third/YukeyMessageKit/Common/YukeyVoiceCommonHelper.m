...
#import "YukeyVoiceCommonHelper.h"

#import "YukeyMacro.h"

@implementation YukeyVoiceCommonHelper
...
+ (void)removeRecordedFileWithOnlyName:(NSString*)fileName block:(DidDeleteAudioFileBlock)block {
    ...
    [YukeyVoiceCommonHelper removeRecordedFile:fileName type:@"amr"];
    
    NSString *originWavFile = [fileName stringByReplacingOccurrencesOfString:@"wavToAmr" withString:@""];
    [YukeyVoiceCommonHelper removeRecordedFile:originWavFile type:@"wav"];
    
    NSString *convertedWavFile = [originWavFile stringByAppendingString:@"amrToWav"];
    [YukeyVoiceCommonHelper removeRecordedFile:convertedWavFile type:@"amr"];
...
+ (void)removeAudioFile:(NSString*)exception block:(DidDeleteAudioFileBlock)block {
    NSString *path = [YukeyVoiceCommonHelper getCacheDirectory];
...
{
    NSString* fileDirectory = [[[YukeyVoiceCommonHelper getCacheDirectory] stringByAppendingPathComponent:_fileName] stringByAppendingPathExtension:_type];
...
{
    NSString* fileDirectory = [[YukeyVoiceCommonHelper getCacheDirectory]stringByAppendingPathComponent:_fileName];
    return fileDirectory;
}
...
+ (void)removeRecordedFile:(NSString*)fileName type:(NSString*)type {
    ...
    NSString *path = [YukeyVoiceCommonHelper getPathByFileName:fileName ofType:type];
