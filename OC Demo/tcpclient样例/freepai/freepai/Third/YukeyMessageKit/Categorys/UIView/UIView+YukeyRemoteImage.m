...
#import "YukeyCacheManager.h"

const char* const kYukeyURLPropertyKey   = "YukeyURLDownloadURLPropertyKey";
const char* const kYukeyLoadingStateKey  = "YukeyURLDownloadLoadingStateKey";
const char* const kYukeyLoadingViewKey   = "YukeyURLDownloadLoadingViewKey";

const char* const kYukeyActivityIndicatorViewKey   = "YukeyActivityIndicatorViewKey";

const char* const kYukeyMessageAvatorTypeKey   = "YukeyMessageAvatorTypeKey";

#define kXHActivityIndicatorViewSize 35

@implementation UIView (YukeyRemoteImage)
...
- (void)setActivityIndicatorView:(UIActivityIndicatorView *)activityIndicatorView {
    objc_setAssociatedObject(self, kYukeyActivityIndicatorViewKey, activityIndicatorView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (UIActivityIndicatorView *)activityIndicatorView {
    return objc_getAssociatedObject(self, kYukeyActivityIndicatorViewKey);
}

- (void)setMessageAvatorType:(YukeyMessageAvatorType)messageAvatorType {
    objc_setAssociatedObject(self, &kYukeyMessageAvatorTypeKey, [NSNumber numberWithInteger:messageAvatorType], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (YukeyMessageAvatorType)messageAvatorType {
    return (YukeyMessageAvatorType)([objc_getAssociatedObject(self, &kYukeyMessageAvatorTypeKey) integerValue]);
}

- (NSURL*)url {
    return objc_getAssociatedObject(self, kYukeyURLPropertyKey);
}
...
- (void)setImageUrl:(NSURL *)url autoLoading:(BOOL)autoLoading {
    if(![url isEqual:self.url]) {
        objc_setAssociatedObject(self, kYukeyURLPropertyKey, url, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (UIImageViewURLDownloadState)loadingState {
    return (NSUInteger)([objc_getAssociatedObject(self, kYukeyLoadingStateKey) integerValue]);
}

- (void)setLoadingState:(UIImageViewURLDownloadState)loadingState {
    objc_setAssociatedObject(self, kYukeyLoadingStateKey, @(loadingState), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (UIView *)loadingView {
    return objc_getAssociatedObject(self, kYukeyLoadingViewKey);
}

- (void)setLoadingView:(UIView *)loadingView {
    [self.loadingView removeFromSuperview];
    
    objc_setAssociatedObject(self, kYukeyLoadingViewKey, loadingView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)loadWithCompletionBlock:(void(^)(UIImage *image, NSURL *url, NSError *error))handler {
    ...
    dispatch_async(self.cachingQueue, ^{
        UIImage *cacheImage = [YukeyCacheManager imageWithURL:weakSelf.url storeMemoryCache:YES];
        if (self.messageAvatorType != YukeyMessageAvatorTypeNormal) {
            cacheImage = [YukeyMessageAvatorFactory avatarImageNamed:cacheImage messageAvatorType:self.messageAvatorType];
        }
        if (cacheImage) {
...
- (void)cachingImageData:(NSData *)imageData url:(NSURL *)url {
    dispatch_async(self.cachingQueue, ^{
        if (imageData) {
            [YukeyCacheManager storeData:imageData forURL:url storeMemoryCache:NO];
            UIImage *image = [UIImage imageWithData:imageData];
            if (image)
                [YukeyCacheManager storeMemoryCacheWithImage:image forURL:url];
        }
    });
}
...
- (UIImage *)didFinishDownloadWithData:(NSData *)data forURL:(NSURL *)url error:(NSError *)error {
    ...
    if (self.messageAvatorType != YukeyMessageAvatorTypeNormal) {
        image = [YukeyMessageAvatorFactory avatarImageNamed:image messageAvatorType:self.messageAvatorType];
    }
    if([url isEqual:self.url]) {
        if(error) {
