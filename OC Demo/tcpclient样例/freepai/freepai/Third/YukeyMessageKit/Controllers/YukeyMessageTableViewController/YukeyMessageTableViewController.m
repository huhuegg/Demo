//
//  YukeyMessageTableViewController.m
//  TestMessageKit
//
...
#import "YukeyMessageTableViewController.h"
#import <MobileCoreServices/MobileCoreServices.h>

@interface YukeyMessageTableViewController ()
...
@property (nonatomic, assign) YukeyInputViewType textViewInputViewType;

@property (nonatomic, weak, readwrite) YukeyMessageTableView *messageTableView;
@property (nonatomic, weak, readwrite) YukeyMessageInputView *messageInputView;
@property (nonatomic, weak, readwrite) YukeyShareMenuView *shareMenuView;
@property (nonatomic, weak, readwrite) YukeyEmotionManagerView *emotionManagerView;
@property (nonatomic, strong, readwrite) YukeyVoiceRecordHUD *voiceRecordHUD;
 ...
 *  管理本机的摄像和图片库的工具对象
 */
@property (nonatomic, strong) YukeyPhotographyHelper *photographyHelper;

/**
 *  管理地理位置的工具对象
 */
@property (nonatomic, strong) YukeyLocationHelper *locationHelper;

/**
 *  管理录音工具对象
 */
@property (nonatomic, strong) YukeyVoiceRecordHelper *voiceRecordHelper;
 ...
 *  @return 返回计算的高度
 */
- (CGFloat)calculateCellHeightWithMessage:(id <YukeyMessageModel>)message atIndexPath:(NSIndexPath *)indexPath;
...
@implementation YukeyMessageTableViewController
...
- (void)addMessage:(YukeyMessage *)addedMessage {
    WEAKSELF
...
- (UIView *)headerContainerView {
    if (!_headerContainerView) {
        _headerContainerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.view.bounds), 50)];
...
- (YukeyShareMenuView *)shareMenuView {
    if (!_shareMenuView) {
        YukeyShareMenuView *shareMenuView = [[YukeyShareMenuView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.view.bounds), CGRectGetWidth(self.view.bounds), self.keyboardViewHeight)];
...
- (YukeyEmotionManagerView *)emotionManagerView {
    if (!_emotionManagerView) {
        YukeyEmotionManagerView *emotionManagerView = [[YukeyEmotionManagerView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.view.bounds), CGRectGetWidth(self.view.bounds), self.keyboardViewHeight)];
...
- (YukeyVoiceRecordHUD *)voiceRecordHUD {
    if (!_voiceRecordHUD) {
        _voiceRecordHUD = [[YukeyVoiceRecordHUD alloc] initWithFrame:CGRectMake(0, 0, 140, 140)];
    }
    return _voiceRecordHUD;
}

- (YukeyPhotographyHelper *)photographyHelper {
    if (!_photographyHelper) {
        _photographyHelper = [[YukeyPhotographyHelper alloc] init];
    }
    return _photographyHelper;
}

- (YukeyLocationHelper *)locationHelper {
    if (!_locationHelper) {
        _locationHelper = [[YukeyLocationHelper alloc] init];
    }
    return _locationHelper;
}

- (YukeyVoiceRecordHelper *)voiceRecordHelper {
    if (!_voiceRecordHelper) {
        WEAKSELF
        _voiceRecordHelper = [[YukeyVoiceRecordHelper alloc] init];
...
- (void)finishSendMessageWithBubbleMessageType:(YukeyBubbleMessageMediaType)mediaType {
    switch (mediaType) {
...
- (void)setup {
    ...
    _allowsSendFace = NO;
    _inputViewStyle = YukeyMessageInputViewStyleFlat;
...
- (void)initilzer {
	...
	YukeyMessageTableView *messageTableView = [[YukeyMessageTableView alloc] initWithFrame:self.view.bounds style:UITableViewStylePlain];
    ...
    CGFloat inputViewHeight = (self.inputViewStyle == YukeyMessageInputViewStyleFlat) ? 45.0f : 40.0f;
    ...
    if (self.allowsPanToDismissKeyboard) {
        ...
        self.messageTableView.keyboardDidScrollToPoint = ^(CGPoint point) {
            if (weakSelf.textViewInputViewType == YukeyInputViewTypeText)
    ...
    // 初始化输入工具条
    YukeyMessageInputView *inputView = [[YukeyMessageInputView alloc] initWithFrame:inputFrame];
...
- (void)viewDidLoad {
    ...
    // 初始化消息页面布局
    [self initilzer];
    [[YukeyMessageBubbleView appearance] setFont:[UIFont systemFontOfSize:16.0f]];
}
...
- (void)layoutAndAnimateMessageInputTextView:(UITextView *)textView {
    CGFloat maxHeight = [YukeyMessageInputView maxHeight];
...
- (CGFloat)calculateCellHeightWithMessage:(id <YukeyMessageModel>)message atIndexPath:(NSIndexPath *)indexPath {
    ...
    cellHeight = [YukeyMessageTableViewCell calculateCellHeightWithMessage:message displaysTimestamp:displayTimestamp];
    ...
    [UIView animateWithDuration:0.2 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        if (hide) {
            switch (self.textViewInputViewType) {
                case YukeyInputViewTypeEmotion: {
                    EmotionManagerViewAnimation(hide);
                    break;
                }
                case YukeyInputViewTypeShareMenu: {
...
- (void)inputTextViewWillBeginEditing:(YukeyMessageTextView *)messageInputTextView {
    self.textViewInputViewType = YukeyInputViewTypeText;
}

- (void)inputTextViewDidBeginEditing:(YukeyMessageTextView *)messageInputTextView {
...
- (void)didChangeSendVoiceAction:(BOOL)changed {
    if (changed) {
        if (self.textViewInputViewType == YukeyInputViewTypeText)
            return;
...
- (void)didSelectedMultipleMediaAction {
    ...
    self.textViewInputViewType = YukeyInputViewTypeShareMenu;
    [self layoutOtherMenuViewHiden:NO];
}

- (void)didSendFaceAction:(BOOL)sendFace {
    if (sendFace) {
        self.textViewInputViewType = YukeyInputViewTypeEmotion;
        [self layoutOtherMenuViewHiden:NO];
    } else {
    ...
    void (^PickerMediaBlock)(UIImage *image, NSDictionary *editingInfo) = ^(UIImage *image, NSDictionary *editingInfo) {
        if (image) {
            ...
            if (CFStringCompare ((__bridge CFStringRef) mediaType, kUTTypeMovie, 0) == kCFCompareEqualTo) {
                ...
                UIImage *thumbnailImage = [YukeyMessageVideoConverPhotoFactory videoConverPhotoWithVideoPath:videoPath];
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    ...
    if (self.textViewInputViewType != YukeyInputViewTypeNormal && self.textViewInputViewType != YukeyInputViewTypeText) {
        [self layoutOtherMenuViewHiden:YES];
    }
}
...
- (id <YukeyMessageModel>)messageForRowAtIndexPath:(NSIndexPath *)indexPath {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    id <YukeyMessageModel> message = [self.dataSource messageForRowAtIndexPath:indexPath];
    ...
    YukeyMessageTableViewCell *messageTableViewCell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    
    if (!messageTableViewCell) {
        messageTableViewCell = [[YukeyMessageTableViewCell alloc] initWithMessage:message displaysTimestamp:displayTimestamp reuseIdentifier:cellIdentifier];
