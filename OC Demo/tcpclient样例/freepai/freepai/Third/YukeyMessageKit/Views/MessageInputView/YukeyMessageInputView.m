...
@interface YukeyMessageInputView()<UITextViewDelegate>
@property (nonatomic, weak, readwrite) YukeyMessageTextView *inputTextView;
 ...
 *  @param style 输入框样式类型
 */
- (void)setupMessageInputViewBarWithStyle:(YukeyMessageInputViewStyle)style ;
...
@implementation YukeyMessageInputView
...
- (UIButton *)createButtonWithImage:(UIImage *)image HLImage:(UIImage *)hlImage {
    UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, [YukeyMessageInputView textViewLineHeight], [YukeyMessageInputView textViewLineHeight])];
    if (image)
...
- (void)setupMessageInputViewBarWithStyle:(YukeyMessageInputViewStyle)style {
    ...
    // 输入框
    CGFloat textViewLeftMargin = ((style == YukeyMessageInputViewStyleFlat) ? 6.0 : 4.0);
    ...
    CGFloat height = [YukeyMessageInputView textViewLineHeight];
    
    // 初始化输入框
    YukeyMessageTextView *textView = [[YukeyMessageTextView  alloc] initWithFrame:CGRectZero];
    ...
    // 配置不同iOS SDK版本的样式
    switch (style) {
        case YukeyMessageInputViewStyleQuasiphysical: {
        ...
        case YukeyMessageInputViewStyleFlat: {
...
- (void)setup {
    ...
    _messageInputViewStyle = YukeyMessageInputViewStyleFlat;
}
...
+ (CGFloat)maxHeight {
    return ([YukeyMessageInputView maxLines] + 1.0f) * [YukeyMessageInputView textViewLineHeight];
}
