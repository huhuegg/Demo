...
#import "YukeyMessageTableViewCell.h"
...
@interface YukeyMessageTableViewCell () {
    
}

@property (nonatomic, weak, readwrite) YukeyMessageBubbleView *messageBubbleView;
 ...
 */
- (void)configureTimestamp:(BOOL)displayTimestamp atMessage:(id <YukeyMessageModel>)message;
 ...
 *  @param message 需要配置的目标消息Model
 */
- (void)configAvatorWithMessage:(id <YukeyMessageModel>)message;
 ...
 *  @param message 需要配置的目标消息Model
 */
- (void)configureMessageBubbleViewWithMessage:(id <YukeyMessageModel>)message;
...
@implementation YukeyMessageTableViewCell
...
- (void)configAvatorWithMessage:(id <YukeyMessageModel>)message {
    if (message.avator) {
        ...
        if (message.avatorUrl) {
            self.avatorButton.messageAvatorType = YukeyMessageAvatorTypeSquare;
            [self.avatorButton setImageWithURL:[NSURL URLWithString:message.avatorUrl] placeholer:[UIImage imageNamed:@"avator"]];
        }
    } else {
        [self.avatorButton setImage:[YukeyMessageAvatorFactory avatarImageNamed:[UIImage imageNamed:@"avator"] messageAvatorType:YukeyMessageAvatorTypeSquare] forState:UIControlStateNormal];
    }
}

- (void)configureMessageBubbleViewWithMessage:(id <YukeyMessageModel>)message {
    YukeyBubbleMessageMediaType currentMediaType = message.messageMediaType;
    ...
    switch (currentMediaType) {
        ...
        case YukeyBubbleMessageMediaTypeEmotion: {
            ...
            if (currentMediaType == YukeyBubbleMessageMediaTypeText) {
            ...
            tapGestureRecognizer.numberOfTapsRequired = (currentMediaType == YukeyBubbleMessageMediaTypeText ? 2 : 1);
...
+ (CGFloat)calculateCellHeightWithMessage:(id <YukeyMessageModel>)message
    ...
    CGFloat avatarHeight = kYukeyAvatarImageSize;
    
    CGFloat subviewHeights = timestampHeight + kXHBubbleMessageViewPadding * 2;
    
    CGFloat bubbleHeight = [YukeyMessageBubbleView calculateCellHeightWithMessage:message];
    ...
    if (self) {
        ...
        switch (message.bubbleMessageType) {
            case YukeyBubbleMessageTypeReceiving:
                avatorButtonFrame = CGRectMake(kXHAvatorPaddingX, kXHAvatorPaddingY + (self.displayTimestamp ? kXHTimeStampLabelHeight : 0), kYukeyAvatarImageSize, kYukeyAvatarImageSize);
                break;
            case YukeyBubbleMessageTypeSending:
                avatorButtonFrame = CGRectMake(CGRectGetWidth(self.bounds) - kYukeyAvatarImageSize - kXHAvatorPaddingX, kXHAvatorPaddingY + (self.displayTimestamp ? kXHTimeStampLabelHeight : 0), kYukeyAvatarImageSize, kYukeyAvatarImageSize);
                break;
            default:
                break;
        }
        ...
        [avatorButton setImage:[YukeyMessageAvatorFactory avatarImageNamed:[UIImage imageNamed:@"avator"] messageAvatorType:YukeyMessageAvatorTypeCircle] forState:UIControlStateNormal];
        ...
        if (!_messageBubbleView) {
            ...
            if (message.bubbleMessageType == YukeyBubbleMessageTypeReceiving)
                bubbleX = kYukeyAvatarImageSize + kXHAvatorPaddingX + kXHAvatorPaddingX;
            else
                offsetX = kYukeyAvatarImageSize + kXHAvatorPaddingX + kXHAvatorPaddingX;
            ...
            // bubble container
            YukeyMessageBubbleView *messageBubbleView = [[YukeyMessageBubbleView alloc] initWithFrame:frame message:message];
...
- (void)layoutSubviews {
    ...
    avatorButtonFrame.origin.x = ([self bubbleMessageType] == YukeyBubbleMessageTypeReceiving) ? kXHAvatorPaddingX : ((CGRectGetWidth(self.bounds) - kXHAvatorPaddingX - kYukeyAvatarImageSize));
    ...
    CGFloat bubbleX = 0.0f;
    if ([self bubbleMessageType] == YukeyBubbleMessageTypeReceiving)
        bubbleX = kYukeyAvatarImageSize + kXHAvatorPaddingX + kXHAvatorPaddingX;
