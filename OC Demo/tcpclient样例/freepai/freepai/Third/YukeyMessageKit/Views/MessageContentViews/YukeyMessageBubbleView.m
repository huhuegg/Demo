...
#import "YukeyMessageBubbleHelper.h"
...
@interface YukeyMessageBubbleView ()
...
@property (nonatomic, weak, readwrite) YukeyBubblePhotoImageView *bubblePhotoImageView;
...
@property (nonatomic, strong, readwrite) id <YukeyMessageModel> message;

@end

@implementation YukeyMessageBubbleView

#pragma mark - Bubble view

+ (CGFloat)neededWidthForText:(NSString *)text {
    CGSize stringSize;
    stringSize = [text sizeWithFont:[[YukeyMessageBubbleView appearance] font]
...
+ (CGSize)neededSizeForText:(NSString *)text {
    ...
    CGFloat dyWidth = [YukeyMessageBubbleView neededWidthForText:text];
    
    CGSize textSize = [SETextView frameRectWithAttributtedString:[[YukeyMessageBubbleHelper sharedMessageBubbleHelper] bubbleAttributtedStringWithText:text] constraintSize:CGSizeMake(maxWidth, MAXFLOAT) lineSpacing:kXHTextLineSpacing font:[[YukeyMessageBubbleView appearance] font]].size;
...
+ (CGSize)neededSizeForVoicePath:(NSString *)voicePath voiceDuration:(NSString *)voiceDuration {
    ...
    CGSize voiceSize = CGSizeMake(100 + (gapDuration > 0 ? (120.0 / (60 - 1) * gapDuration) : 0), [YukeyMessageInputView textViewLineHeight]);
    return voiceSize;
}

+ (CGFloat)calculateCellHeightWithMessage:(id <YukeyMessageModel>)message {
    CGSize size = [YukeyMessageBubbleView getBubbleFrameWithMessage:message];
    return size.height + kMarginTop + kMarginBottom;
}

+ (CGSize)getBubbleFrameWithMessage:(id <YukeyMessageModel>)message {
    CGSize bubbleSize;
    switch (message.messageMediaType) {
        case YukeyBubbleMessageMediaTypeText: {
            bubbleSize = [YukeyMessageBubbleView neededSizeForText:message.text];
            break;
        }
        case YukeyBubbleMessageMediaTypePhoto: {
            bubbleSize = [YukeyMessageBubbleView neededSizeForPhoto:message.photo];
            break;
        }
        case YukeyBubbleMessageMediaTypeVideo: {
            bubbleSize = [YukeyMessageBubbleView neededSizeForPhoto:message.videoConverPhoto];
            break;
        }
        case YukeyBubbleMessageMediaTypeVoice: {
            // 这里的宽度是不定的，高度是固定的，根据需要根据语音长短来定制啦
            bubbleSize = [YukeyMessageBubbleView neededSizeForVoicePath:message.voicePath voiceDuration:message.voiceDuration];
            break;
        }
...
- (CGRect)bubbleFrame {
    CGSize bubbleSize = [YukeyMessageBubbleView getBubbleFrameWithMessage:self.message];
    
    return CGRectIntegral(CGRectMake((self.message.bubbleMessageType == YukeyBubbleMessageTypeSending ? CGRectGetWidth(self.bounds) - bubbleSize.width : 0.0f),
                                     kMarginTop,
                                     bubbleSize.width,
...
- (void)configureBubbleImageView:(id <YukeyMessageModel>)message {
    YukeyBubbleMessageMediaType currentType = message.messageMediaType;
    ...
    switch (currentType) {
        ...
        case YukeyBubbleMessageMediaTypeText:
        case YukeyBubbleMessageMediaTypeEmotion: {
            _bubbleImageView.image = [YukeyMessageBubbleFactory bubbleImageViewForType:message.bubbleMessageType style:YukeyBubbleImageViewStyleWeChat meidaType:message.messageMediaType];
            ...
            if (currentType == YukeyBubbleMessageMediaTypeText) {
                ...
                if (currentType == YukeyBubbleMessageMediaTypeVoice) {
                    [_animationVoiceImageView removeFromSuperview];
                    _animationVoiceImageView = nil;
                    
                    UIImageView *animationVoiceImageView = [YukeyMessageVoiceFactory messageVoiceAnimationImageViewWithBubbleMessageType:message.bubbleMessageType];
        ...
        case YukeyBubbleMessageMediaTypeLocalPosition: {
            ...
            _videoPlayImageView.hidden = (currentType != YukeyBubbleMessageMediaTypeVideo);
            
            _geolocationsLabel.hidden = (currentType != YukeyBubbleMessageMediaTypeLocalPosition);
...
- (void)configureMessageDisplayMediaWithMessage:(id <YukeyMessageModel>)message {
    switch (message.messageMediaType) {
        case YukeyBubbleMessageMediaTypeText:
            _displayTextView.attributedText = [[YukeyMessageBubbleHelper sharedMessageBubbleHelper] bubbleAttributtedStringWithText:[message text]];
            break;
        case YukeyBubbleMessageMediaTypePhoto:
...
- (instancetype)initWithFrame:(CGRect)frame
                      message:(id <YukeyMessageModel>)message {
    ...
    if (self) {
        ...
        if (!_displayTextView) {
            ...
            displayTextView.font = [[YukeyMessageBubbleView appearance] font];
        ...
        if (!_bubblePhotoImageView) {
            YukeyBubblePhotoImageView *bubblePhotoImageView = [[YukeyBubblePhotoImageView alloc] initWithFrame:CGRectZero];
...
- (void)layoutSubviews {
    ...
    YukeyBubbleMessageMediaType currentType = self.message.messageMediaType;
    ...
    switch (currentType) {
        ...
        case YukeyBubbleMessageMediaTypeEmotion: {
            ...
            if (self.message.bubbleMessageType == YukeyBubbleMessageTypeReceiving) {
            ...
            animationVoiceImageViewFrame.origin = CGPointMake((self.message.bubbleMessageType == YukeyBubbleMessageTypeReceiving ? (bubbleFrame.origin.x + kVoiceMargin) : (bubbleFrame.origin.x + CGRectGetWidth(bubbleFrame) - kVoiceMargin - CGRectGetWidth(animationVoiceImageViewFrame))), 17);
...
- (void)resetVoiceDurationLabelFrameWithBubbleFrame:(CGRect)bubbleFrame {
    ...
    voiceFrame.origin.x = (self.message.bubbleMessageType == YukeyBubbleMessageTypeSending ? bubbleFrame.origin.x - _voiceDurationLabel.frame.size.width : bubbleFrame.origin.x + bubbleFrame.size.width);
    _voiceDurationLabel.frame = voiceFrame;
    
    _voiceDurationLabel.textAlignment = (self.message.bubbleMessageType == YukeyBubbleMessageTypeSending ? NSTextAlignmentRight : NSTextAlignmentLeft);
}
